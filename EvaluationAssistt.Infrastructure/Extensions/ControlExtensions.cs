...
namespace EvaluationAssistt.Infrastructure.Extensions
{
    public static class ControlExtensions
    {
        public static void Clear(this System.Web.UI.WebControls.Panel panel)
        {
            ...
            var combobox_list =
            (from p in panel.Controls.OfType<DevExpress.Web.ASPxComboBox>()
              select p).ToList();

            combobox_list.ForEach(x => x.SelectedItem = null);

            var spinedit_list =
            (from p in panel.Controls.OfType<DevExpress.Web.ASPxSpinEdit>()
              select p).ToList();
            ...
            var listbox_list =
            (from p in panel.Controls.OfType<DevExpress.Web.ASPxListBox>()
              select p).ToList();
